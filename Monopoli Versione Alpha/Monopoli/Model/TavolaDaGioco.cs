...
namespace Monopoli.Model
{
    public class TavolaDaGioco
    {
        ...
        public virtual void ResetTavolaDaGioco()
        {
            foreach (Casella casella in Caselle)
            {
                if (casella is TerrenoNormale)
                {
                    TerrenoNormale tn = (TerrenoNormale)casella;
                    tn.Proprietario = null;
                    tn.ResetEdifici();
                    tn.AggiornaAffitto(_gruppi[tn.NomeGruppo]);
                }
                else if (casella is TerrenoSpeciale)
                {
                    TerrenoSpeciale ts = (TerrenoSpeciale)casella;
                    ts.Proprietario = null;
                    ts.AggiornaAffitto(_gruppi[ts.NomeGruppo]);
                }
            }
        }
        #endregion
